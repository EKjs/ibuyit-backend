...
export const updateFavAd = asyncHandler(async (req, res) => {
  ...
  if (!Number.isInteger(favId))
    throw new ErrorResponse("Bad request", 400);
  const { favAdId,description } = req.body;
  const runQuery = "UPDATE ONLY favads SET ad_id=$1, description=$2 WHERE id=$3 AND user_id=$4 RETURNING id"; //here will be permissions check
...
export const deleteFavAd = asyncHandler(async (req, res) => {
  ...
  if (!Number.isInteger(favId))
    throw new ErrorResponse("Bad request", 400);
  const runQuery = "DELETE FROM ONLY favads WHERE id=$1 AND user_id=$2 RETURNING id";//here will be permissions check
